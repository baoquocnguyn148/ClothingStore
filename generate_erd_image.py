...
# Ensure auth.users exists for relationships
...
puml.append("skinparam ranksep 40")
...
for from_table, from_col, to_table, to_col in foreign_keys:
    # A simple line for the relationship
    # If the column is unique or primary key, it might be 1:1, else 1:N
    puml.append(f"{to_table} ||--o{{ {from_table} : \"{from_col}\"")

puml.append("@enduml")
puml_text = "\n".join(puml)

print("Generated PlantUML ERD. Sending to Kroki...")
try:
    ...
    with open('erd_database.png', 'wb') as f:
        f.write(png_data)
    print("Saved ERD to erd_database.png")
