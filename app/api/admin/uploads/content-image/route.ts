...
// POST /api/admin/uploads/content-image
// form-data: file, folder (optional), name (optional)
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const folder = String(formData.get('folder') ?? 'content');
    const name = String(formData.get('name') ?? 'content');

    if (!(file instanceof File)) return jsonError('File is required', 400);
    if (!ALLOWED_TYPES.has(file.type)) return jsonError('Only JPG, PNG, WEBP or AVIF images are allowed', 422);
    if (file.size > MAX_FILE_SIZE) return jsonError('Image must be 5MB or smaller', 422);

    const db = createAdminClient();
    const extension = ALLOWED_TYPES.get(file.type);
    const fileName = `${safeName(file.name) || 'image'}-${crypto.randomUUID()}.${extension}`;
    const path = `${safeName(folder) || 'content'}/${safeName(name) || 'content'}/${fileName}`;
    ...
    if (uploadError && uploadError.message.toLowerCase().includes('bucket')) {
      await db.storage.createBucket('content-images', { public: true });
